...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, **kwargs):
        ...
        features, pos = self.backbone(samples, points=points)
...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        target_quality = torch.cat([
            t["quality"][i] if "quality" in t else torch.ones_like(t["boxes"][:, 0])[i]
        ...
        precision = torch.exp(target_quality)
        loss_iou = 1 - torch.diag(
            box_ops.box_iou_union(
                box_ops.box_cxcywh_to_xyxy(src_boxes),
                box_ops.box_cxcywh_to_xyxy(target_boxes),
            )[0]
        )
        loss_uncertainty_iou = (precision * loss_iou) - target_quality + 1.0

        losses['loss_uncertainty_box'] = loss_uncertainty_iou.sum() / num_boxes
        return losses
...
def build(args):
    ...
    weight_dict['loss_uncertainty_box'] = args.loss_uncertainty_box
    if args.masks:
