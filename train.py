...
def train_one_epoch(
    ...
    for step, samples in enumerate(dataloader):
        ...
        if detr_num_train_frames > 0:
            train_target_idxs = detr_outputs_flatten_idxs[:_B * detr_num_train_frames]
            detr_train_targets = [detr_targets_flatten[i] for i in train_target_idxs.tolist()]

            if detr_num_checkpoint_frames == 0 or detr_num_checkpoint_frames >= len(detr_train_frames):
                train_points = [t['boxes'][:, :2] for t in detr_train_targets]
                detr_train_outputs = model(frames=detr_train_frames, part="detr", points=train_points)
            else:
                detr_train_outputs = None
                for i, batch_samples in enumerate(batch_iterator(
                    detr_num_checkpoint_frames,
                    detr_train_frames,
                )):
                    batch_frames = batch_samples[0]

                    curr_start = i * detr_num_checkpoint_frames
                    curr_end = curr_start + len(batch_frames)
                    batch_targets = detr_train_targets[curr_start : curr_end]

                    batch_points = [t['boxes'][:, :2] for t in batch_targets]
...
def annotations_to_flatten_detr_targets(annotations: list, device):
    targets = []
    for annotation in annotations:      # 遍历 batch
        for ann in annotation:          # 遍历 frame
            # 1. 先构建基础的 target 字典
            target = {
                "boxes": ann["bbox"].to(device),
                "labels": ann["category"].to(device),
            ...
            if "quality" in ann:
                target["quality"] = ann["quality"].to(device)
